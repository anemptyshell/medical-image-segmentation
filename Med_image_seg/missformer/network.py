...
class BridgeLayer_4(nn.Module):
    ...
    def forward(self, inputs):
        # print(inputs[0].shape)    ## torch.Size([4, 64, 56, 56])
        B = inputs[0].shape[0]
        C = 64
        if (type(inputs) == list):
            # print("-----1-----")
            ...
            ## torch.Size([4, 64, 56, 56]) torch.Size([4, 128, 28, 28]) torch.Size([4, 320, 14, 14]) torch.Size([4, 512, 7, 7])
            B, C, _, _= c1.shape
            ...
            ## torch.Size([4, 3136, 64]) torch.Size([4, 1568, 64]) torch.Size([4, 980, 64]) torch.Size([4, 392, 64])
        ...
        # print('tx1.size()')
        # print(tx1.size())   # torch.Size([4, 6076, 64])

        tx = self.norm2(tx1)
        # print('tx.size()')
        # print(tx.size())      # torch.Size([4, 6076, 64])
        # print('*************')

        tem1 = tx[:,:3136,:].reshape(B, -1, C) 
        # print('tem1.size()')
        # print(tem1.size())    # torch.Size([4, 3136, 64])

        tem2 = tx[:,3136:4704,:].reshape(B, -1, C*2)
        # print('tem2.size()')
        # print(tem2.size())    # torch.Size([4, 784, 128])

        tem3 = tx[:,4704:5684,:].reshape(B, -1, C*5)
        # print('tem3.size()')
        # print(tem3.size())    # torch.Size([4, 196, 320])

        tem4 = tx[:,5684:6076,:].reshape(B, -1, C*8)
        # print('tem4.size()')
        # print(tem4.size())    # torch.Size([4, 49, 512])
        # print('*************')
        ...
        # print('t1.size()')
        # print(t1.size())   # torch.Size([4, 6076, 64])
        
        tx2 = tx1 + t1
        # print('tx2.size()')
        # print(tx2.size())       # torch.Size([4, 6076, 64])
        # print('=====================')
...
class BridegeBlock_4(nn.Module):    ## 跳跃连接
    def __init__(self, dims, head, reduction_ratios):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # print(sk1.size(),sk2.size(),sk3.size(),sk4.size())
...
# missformer = MISSFormer(num_classes=1)
# a = torch.rand(4, 3, 512, 512)
# output = missformer.forward(a)
# print('------------------')
# print('output.size()')
# print(output.size())    # torch.Size([4, 1, 224, 224])

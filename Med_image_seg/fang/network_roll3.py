...
class Lo2(nn.Module):
    ...
    def _init_weights(self, m):
        ...
        if isinstance(m, nn.Linear):
            ## 对于线性层（nn.Linear），使用截断正态分布初始化权重，并根据情况初始化偏置为 0。
    ...
    def forward(self, x, H, W):
        ...
        ## 将 xn 按照通道维度（维度 1）分割成 C 个张量，每个张量的形状为 (B, 1, H, W)
        xs = torch.chunk(xn, C, 1)

        x_shift = [torch.roll(x_c, shift, 2) for x_c, shift in zip(xs, range(0, C))]     ## shift 张量元素移位的位数
        x_cat = torch.cat(x_shift, 1)         ## 将滚动后的张量在通道维度上进行拼接
        ...
        x3 = torch.cat([x1, x2], dim=2) ##通道维度上拼接
        x3 = self.fc6(x3)
        x3 = self.drop(x3)
        return x3
...
class Rolling_Unet_L(nn.Module):
    ...
    def forward(self, x):
        ...
        out1_1 = torch.add(out, t3)
        out1_2 = torch.add(out, t3)
        out1_3 = torch.add(out, t3)

        out1_1 = F.interpolate(self.decoder3(out1_1), scale_factor=(2, 2), mode='bilinear')
        out1_2 = F.interpolate(self.decoder3(out1_2), scale_factor=(2, 2), mode='bilinear')
        out1_3 = F.interpolate(self.decoder3(out1_3), scale_factor=(2, 2), mode='bilinear')

        out2_1 = torch.add(out1_1, t2)
        out2_2 = torch.add(out1_2, t2)
        out2_3 = torch.add(out1_3, t2)

        out2_1 = F.interpolate(self.decoder2(out2_1), scale_factor=(2, 2), mode='bilinear')
        out2_2 = F.interpolate(self.decoder2(out2_2), scale_factor=(2, 2), mode='bilinear')
        out2_3 = F.interpolate(self.decoder2(out2_3), scale_factor=(2, 2), mode='bilinear')

        out1 = torch.add(out2_1, t1)
        out2 = torch.add(out2_2, t1)
        out3 = torch.add(out2_3, t1)
...
#     model = Rolling_Unet_L(num_classes=1, img_size=256)
#     input = torch.rand(2, 3, 256, 256)
#     out1, out2, out3 = model.forward(input)
#     print(out1.size())   ## torch.Size([2, 1, 512, 512])
...
"""unet3de + 融合"""
class UNet_Multi_decoder_fusion(nn.Module):
    def __init__(self, input_ch=3, output_ch=1):
        super(UNet_Multi_decoder_fusion, self).__init__()
    ...
    # def forward(self, x5, x4):
        ...
        d5_1_fa, d5_2_fa = self.fa_l1(d5_1, d5_2)
        # print(d5_1_fa.size())   ## ([2, 512, 64, 64])
        # print(d5_2_fa.size())   ## ([2, 512, 64, 64])


        # #####################################
        d4_1 = self.Up4(d5_1_fa)
        d4_1 = torch.cat((x3, d4_1), dim=1)  
        d4_1 = self.Up_conv4(d4_1)          ## torch.Size([2, 256, 128, 128])

        d4_2 = self.Up4(d5_2_fa)
        d4_2 = torch.cat((x3, d4_2), dim=1)  
        d4_2 = self.Up_conv4(d4_2)  

        d4_1_fa, d4_2_fa = self.fa_l2(d4_1, d4_2)

        
        # #####################################
        d3_1 = self.Up3(d4_1_fa)
        d3_1 = torch.cat((x2, d3_1), dim=1)
        d3_1 = self.Up_conv3(d3_1)          ## torch.Size([2, 128, 256, 256])

        d3_2 = self.Up3(d4_2_fa)
        d3_2 = torch.cat((x2, d3_2), dim=1)
        d3_2 = self.Up_conv3(d3_2)  

        d3_1_fa, d3_2_fa = self.fa_l3(d3_1, d3_2)


        # #####################################
        d2_1 = self.Up2(d3_1_fa)
        d2_1 = torch.cat((x1, d2_1), dim=1)
        d2_1 = self.Up_conv2(d2_1)          ## torch.Size([2, 64, 512, 512]) 

        d2_2 = self.Up2(d3_2_fa)
        d2_2 = torch.cat((x1, d2_2), dim=1)
        d2_2 = self.Up_conv2(d2_2) 

        d2_1_fa, d2_2_fa = self.fa_l4(d2_1, d2_2)

        # #####################################

        d1_1 = self.Conv_1x1(d2_1_fa)          ## torch.Size([2, 1, 512, 512]) 
        d1_2 = self.Conv_1x1(d2_2_fa) 
        
        out = torch.cat((d1_1, d1_2), dim=1)
        out = self.fa_l5(out)
 
        return out, d1_1, d1_2
    ## 最终输出, 边界输出, 骨架输出
...
"""随机张量测试"""    
# unet = UNet_Multi_decoder_fusion()

# a = torch.rand(2, 3, 512, 512)
# # output1 = unet.forward(a)
# out, d1_1, d1_2 = unet.forward(a)

# print(out.size())   # torch.Size([2, 1, 512, 512])
# # print(output2.size()) 
# # print(output3.size()) 

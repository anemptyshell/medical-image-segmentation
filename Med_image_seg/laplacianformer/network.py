...
from Med_image_seg.laplacianformer.utils import *
...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        # print(x.shape)  # torch.Size([4, 256, 512])
        H, W = self.input_resolution
        x = self.expand(x)  
        # print(x.shape)   # torch.Size([4, 256, 1024])
        B, L, C = x.shape
        # print(L)   # 256
        # print(H)   # 7
        # print(W)   # 7
...
class LaplacianFormer(nn.Module):
    def __init__(self, num_classes=1, n_skip_bridge=1, pyramid_levels=3, token_mlp_mode="mix_skip"):
        super().__init__()
...
# net = LaplacianFormer(num_classes=1, n_skip_bridge=1, pyramid_levels=3).to('cuda')
# a = torch.rand(4, 3, 224, 224).to('cuda')
# output = net.forward(a)
# # print('------------------')
# print('output.size()')
# print(output.size())   # torch.Size([4, 1, 224, 224])

## 如果输入是512 * 512，则需要将 d_base_feat_size = 16

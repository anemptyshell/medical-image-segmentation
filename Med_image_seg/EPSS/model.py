...
class EPSS(base_model):
    ...
    def train_epoch(self, train_loader, epoch, step):
        ...
        for iter, data in enumerate(train_loader):
            ...
            loss2 = self.BceDiceLoss(edge_pred, edge)
            loss_mi = (loss_mi1 + loss_mi2 + loss_mi3 + loss_mi4) / 4.0
            loss = loss1 + 0.5*loss2 + 0.5*loss_mi
    ...
    def visualize_single_channel(self, feature_map, save_dir, iter, channel_idx=0, size=512):
        """
        可视化特征图的单个通道（类似你的预测图风格）
        ...
        """
        # 取指定通道
        channel_data = feature_map[0, channel_idx].detach().cpu().numpy()
        ...
        # 使用viridis色彩映射（比gray更好显示特征）
        plt.imshow(channel_norm, cmap='viridis')
        plt.axis('off')

        # 保存
        filename = f'{save_dir}/iter_{iter}_channel_{channel_idx}.png'
